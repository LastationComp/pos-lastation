import { responseError, responseSuccess } from "@/app/_lib/handleResponse";
...
export async function POST(
  ...
  const { license_key, new_password, re_password, old_password } =
    await req.json();

  if (new_password.length < 8) return responseError("Password length is min 8");
  if (new_password !== re_password)
    return responseError("New Password and Re Password is not match");
  ...
  if (!checkAdmins || !new_password || !old_password)
    return responseError("Please check your credentials");
